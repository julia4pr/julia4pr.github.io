...
```markdown
`胆小鬼大赢家` 具有非常强大的邀请关系跟踪功能

把你的 `KEY` 告诉你的朋友，或者使用你的钱包浏览器打开游戏 `Me` 界面，把生成的邀请链接给他，都可以有效完成邀请

如果你已经是玩家，浏览游戏任意页面时 `复制网址` 或者在 imToken 钱包中 `分享页面` 都可以自动跟踪推荐关系

总之，只要有人访问带着你 `KEY` 的，有 `?a=0x..` 参数的网址，到 [留言页]({base_url}/push) 步骤时，页面表单中都会自动填上你的 `KEY` 钥匙
```
